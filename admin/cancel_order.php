...
$dateupdate = date("Y-m-d H:i:s");
	...
	if($_GET['action'] == 'delete')
	{
		...
		mysqli_query($mysqli,"delete from `tbl_dispatch_detail` Where `dispatch_id`='".$_GET['id']."' and `status`='4'");
		echo "<script language='javascript'>window.location='cancel_order.php?msg=deleted';</script>";
	...
	if(isset($_POST['submit']))
	{
		...
		if($_POST['action'] == 'edit')
		{
			...
			echo "<script language='javascript'>window.location='cancel_order.php';</script>";
...
<div class="text-center mt-4">
    ...
    <button type="reset" class="btn btn-danger" onclick="window.location='cancel_order.php';">Cancel</button>
</div>
			<?php
            }
			else
                            ...
                            <div class="panel panel-default gradient">
                              <!-- Build page from here: -->
                                ...
                                <div class="panel-body noPad clearfix">
									...
									$sql_forms = mysqli_query($mysqli,"select ord.*,us.name,us.mobile_no,us.name from `tbl_dispatch_order` ord,`tbl_user` us where us.id=ord.user_id and ord.`status`='4' $extrasearch $qrysearch");
									$pages->items_total = $sql_forms->num_rows;
									$pages->mid_range = 9;
									$pages->paginate();	

									$result	= mysqli_query($mysqli,"select ord.*,us.name,us.mobile_no,us.name from `tbl_dispatch_order` ord,`tbl_user` us where us.id=ord.user_id and ord.`status`='4' $extrasearch $qrysearch ORDER BY ord.id DESC ".$pages->limit."");
										...
										if($pages->items_total>0){
											...
											elseif($row['status'] == '4')
											{
												$status = ' <span style="color: Red;"><span style="color: Red;">Cancel</span>';
											}
											...
											$subquery = "SELECT * FROM `tbl_dispatch_detail` WHERE `dispatch_id`='".$row['id']."' and `status`='4'";
										 ...
										 ?>
                                            <tr class="odd gradeX <?php echo $success;?>">
                                          	...
                                          	<!--  <a href="mpdf60/examples/dispatch_order.php?id=<?php echo $row['id']; ?>" target="_blank" class="tip" title="Print Order Form"><span class="icon24 icomoon-icon-print"></span></a>-->
                                          	 </td>
												  ...
												  <div class="modal-dialog" role="document">
													...
													<div class="modal-content">
													  <div class="modal-header">
														<h3 class="modal-title" id="modalLabel<?php echo $row['id']; ?>">Cancel Order Detail
                                              ...
                                              <td><a class="tip" href="cancel_order.php?action=delete&amp;id=<?php echo $row['id'];?>" onClick="return confirmdelete();"
