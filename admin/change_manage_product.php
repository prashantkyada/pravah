...
$file_path = 'http://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['REQUEST_URI']).'/';
	...
	if($_GET['action'] == 'delete')
	{
		...
		echo "<script language='javascript'>window.location='change_manage_product.php?msg=deleted';</script>";
	}
	if($_GET['upd_status'] == 'act')
	{
		mysqli_query($mysqli,"update `tbl_product` set status='1' where `id`='".$_GET['id']."'");
		echo "<script language='javascript'>window.location='change_manage_product.php';</script>";
		die;
	}
	...
	if(isset($_POST['submit']))
	{
		...
		{	
			...
			if(!empty($sizes) && is_array($sizes)) 
			{
				for($i = 0; $i < count($sizes); $i++) 
				{
					$size = $sizes[$i];
					$price = $prices[$i];
					$std_packing = $std_packings[$i];
					$min_qty = $min_qtys[$i];
					
					mysqli_query($mysqli,"insert `tbl_product_detail` set 
					`product_id`='".$last_id."',
					`size`='".$size."',
					`price`='".$price."',
					`std_paking`='".$std_packing."',
					`min_qty`='".$min_qty."'
					");
				}
			}
			echo "<script language='javascript'>window.location='change_manage_product.php?action=added';</script>";
		}
		...
		{	
			...
			if (isset($_POST["size"])) 
			{
				// Remove empty values from the size array
				$sizes = array_filter($_POST["size"], function ($value) 
				{
			...
			$std_packings = $_POST["std_paking"];
			$min_qtys = $_POST["min_qty"];
			$product_detail_ids = $_POST['product_detail_id'];
			...
			if (!empty($sizes) && is_array($sizes)) {
				...
				for ($i = 0; $i < count($sizes); $i++) {
					...
					$std_packing = $std_packings[$i];
					$min_qty = $min_qtys[$i];
					$product_detail_id = $product_detail_ids[$i];
				
					$check_query = mysqli_query($mysqli, "SELECT * FROM `tbl_product_detail` WHERE id='".$product_detail_id."'");
					if(mysqli_num_rows($check_query)> 0)
					{	
						
						// Update existing record
						mysqli_query($mysqli,"UPDATE `tbl_product_detail` set 
						`size`='".$size."',
						`price`='".$price."',
						`std_paking`='".$std_packing."',
						`min_qty`='".$min_qty."'
						where `id`='".$product_detail_id."'");
						echo "<script language='javascript'>window.location='change_manage_product.php?msg=$product_detail_id';</script>";
					}
					else
					{
						//Insert new record
			...
			echo "<script language='javascript'>window.location='change_manage_product.php?msg=updated';</script>";
...
?>
	...
	if($_GET['action'] == "edit")
	{
		...
		$edit_detail_query = mysqli_query($mysqli, "SELECT * FROM `tbl_product_detail` WHERE product_id='".$upd_row['id']."'");
		while ($upd_detail_row = mysqli_fetch_assoc($edit_detail_query)) {
				$upd_detail_rows[] = $upd_detail_row; // Store each row in the array
			}
		
		$first_index = 0; // First index is always 0
		if($upd_detail_rows !='')
		{	
			$last_index = count($upd_detail_rows); // Get last index
		}
	...
	?>	
     <!-- Build page from here: -->
		...
		$counter = '0';	
		if (is_array($upd_detail_rows) && !empty($upd_detail_rows)) { 			
			foreach ($upd_detail_rows as $index => $row) {
...
?>
<input type="hidden" name="product_detail_id[]" value="<?php echo $row['id']; ?>" />
<div class="row-item form-group" id="row-<?php echo $count; ?>">
    <label class="col-lg-2 control-label" for="tags">Size:</label>
    <div class="col-lg-4">
        <input name="size[]" class="form-control" type="text" value='<?php echo $row['size']; ?>' />
    </div>

    <label class="col-lg-2 control-label" for="tags">Price:</label>
    <div class="col-lg-4">
        <input name="price[]" class="form-control" type="text" value="<?php echo  $row['price']; ?>" />
    </div>

    <div class="form-group">
        <label class="col-lg-2 control-label" for="tags">Master Pack (Pcs. / Cartoon):</label>
        <div class="col-lg-4">
            <input name="std_paking[]" class="form-control" type="text" value="<?php echo $row['std_paking']; ?>" />
        </div>
        <label class="col-lg-2 control-label" for="tags">Avg. Weight (Kg. / Pcs.):</label>
        <div class="col-lg-4">
            <input name="min_qty[]" class="form-control" type="text" value="<?php echo $row['min_qty']; ?>" />
        </div>
    </div>
...
if ($counter == $last_index) { ?>
    ...
    <?php }?>
    <button type="button" class="btn btn-danger delete-btn" onClick="deleteRow(this,<?php echo $row['id']; ?>)">Delete</button>
</div>
				<?php
					...
					{
					?>
						<input type="hidden" name="product_detail_id[]" value="0" />	
                          ...
                          <div class="form-group" id="inputContainer">
                           ...
                           <div class="form-group">
                                <div class="col-lg-offset-2">
                                    ...
                                    <button type="reset" class="btn btn-danger" onclick="javascript:window.location='change_manage_product.php';">Cancel</button>
                                </div>
                            </div><!-- End .form-group  -->
    					   </form>
                    </div><!-- End .span12 -->
...
function OnButtonClick() {
  ...
  newDiv.innerHTML = `
    ...
    <div class="col-lg-2">
      <button type="button" class="btn btn-danger delete-btn" onClick="deleteRow(this,0)">Delete</button>
    </div>
  `;
...
if (!confirm("Are you sure you want to delete this row?")) {
	...
	{	
		  //end AJAX request to delete from the database
			fetch("ajex.php", {
				method: "POST",
				headers: {
                               ...
                               <!-- Build page from here: -->
                                ...
                                <div class="panel-heading">
                                   <h4 style="text-align: center;">
                                       <button class="btn btn-info" onclick="javascript:window.location='change_manage_product.php?action=add';">
                                         Add Product
                                     </button>
                                    </h4>
                                </div>
								...
								$aow = ($_GET['ow'] == 'desc') ? "desc" : "desc" ;
								$order_way = ($_GET['ow'] == '') ? "asc" : $_GET['ow'];
								?>
                                ...
                                <div class="panel-body noPad clearfix">
                                    <table cellpadding="0" cellspacing="0" id="checkAll" class="display table table-bordered" width="100%">
										...
										$sql_forms = mysqli_query($mysqli,"select tq.*,ca.category_name
											from `tbl_product` tq,`tbl_category` ca where ca.id=tq.category_id $qrysearch");
										...
										$pages->paginate();	
                                         ...
                                         <tr class="odd gradeX">
                                                ...
                                                <img src="../image/product/<?php echo $row['image']; ?>"
												...
												{
												?>
												 <img src="images/logo.png" alt="Logo" width='60' height='60' class="image marginR10"/>
												<?php
												}
												?>
												</td>
                                                <td><?php echo $row['date']; ?></td>
                                                <td class="center"><a class="listinglingk" href="change_manage_product.php?upd_status=<?php echo $new_status_kwd;?>&id=<?php echo $row['id'];?>"><strong><?php echo $new_status;?></strong></a></td>
                                                <td class="controls center">
                                                <a class="tip" href="change_manage_product.php?action=edit&amp;id=<?php echo $row['id'];?>" 
                                                title="Edit task" data-hasqtip="true" aria-describedby="qtip-7">
                                               <span class="icon12 icomoon-icon-pencil"></span>
                                               </a>&nbsp;&nbsp;
                                               <a class="tip" href="change_manage_product.php?action=delete&amp;id=<?php echo $row['id'];?>" onClick="return confirmdelete();"
