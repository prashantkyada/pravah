...
function generateHeader($date, $order_id, $subdata,$gst_no,$city_name){
     return '<table class="order-form"  style="font-family: serif; font-size: 12pt;" border="0" cellspacing="0" cellpadding="5" width="100%">
...
JOIN 
    tbl_user u ON d.user_id = u.id
JOIN 
    tbl_dispatch_detail dd ON d.id = dd.dispatch_id
JOIN 
    tbl_product p ON dd.product_id = p.id
JOIN 
    tbl_product_detail pd ON dd.product_dtid = pd.id
WHERE 
    d.status = 2
    AND d.id = '".$id."'  -- Replace with the specific dispatch order ID
...
while ($subdata = mysqli_fetch_array($subsql)) {
    ...
    if ($row_count % $page_rows === 0) {
        ...
        $html .= '<table class="form-table" border="1" style="font-family: serif; font-size: 14pt; border-collapse: collapse; border-radius: 10px; width:100%; ">
            <thead>
                <tr>
   ...
   $html .= '<tr style="line-height: 1.4;">
    <td style="line-height: 1.4;">' . $product_name . ' Size : ' . $subdata['product_size'] . '</td>
...
$mpdf->WriteHTML($html);
$mpdf->Output();
$filename = 'dispatch-Order-' . $order_id . '_' . date("d-m-Y") . '.pdf';
$mpdf->Output($filename, 'I'); // Or 'D' if you want to force download
