...
{  
   ...
   $link = 'https://';
   $pdf_path = 'https://'.$_SERVER[ 'SERVER_NAME' ].'/admin/mpdf60/order_form/';
   $order_form_api_path = 'https://'.$_SERVER[ 'SERVER_NAME' ].'/Api/mpdf60/order_form/';
}
else
{
  //$link = 'http://'.$_SERVER['SERVER_NAME'].dirname($_SERVER['REQUEST_URI']).'/images/';
  $file_path = 'http://'.$_SERVER[ 'SERVER_NAME' ].'/image/';
  $link = 'http://';
  $pdf_path = 'http://'.$_SERVER[ 'SERVER_NAME' ].'/admin/mpdf60/order_form/';
  $order_form_api_path = 'http://'.$_SERVER[ 'SERVER_NAME' ].'/Api/mpdf60/order_form/';
}
...
if(isset($_POST['category_list'])){
	...
	while($data = mysqli_fetch_array( $sql ) ) 
	{
		...
		if($data['image'] !='')
		{
			$row['category_image'] = $file_path.'category/'.$data['image'];
		}
		else
		{
			$row['category_image'] = $file_path.'dummy.png';
		}
...
if(isset($_POST['product_list']) && isset($_POST['category_id']) && isset($_POST['user_id']))
{
	...
	while($data = mysqli_fetch_array($sql)) 
	{
		...
		if($data['image'] !='')
		{
			$row['image'] = $file_path.'product/'.$data['image'];
		}
		else
		{	
			$row['image'] = $file_path.'pravah.png';
		}
		if($data['image'] !='')
		{
			$row['thumb_image'] = $file_path.'product/thumb/'.$data['image'];
		}
		else
		{	
			$row['thumb_image'] = $file_path.'pravah.png';
		}
...
if(isset($_POST['banner_list'])) {
	...
	while($data = mysqli_fetch_array( $sql_how_use ))
	{
		...
		$row['image'] = $file_path.'banner/'.$data['image'];
		array_push($jsonObj,$row);
...
if(isset($_POST['new_order_list']) && isset($_POST['user_id'])) 
{
		...
		if (mysqli_num_rows($mainsql) > 0) {  
			while ($maindata = mysqli_fetch_array($mainsql)) {  
				...
				if($curr_user_row['user_status'] == '2')
				{
					$order_form_link = $order_form_api_path.'order_form.php?order_id='.$maindata['order_id'];
				}
				else
				{
					$order_form_link = '';
...
if(isset($_POST['dispatch_order_list']) && isset($_POST['user_id'])) 
{
		...
		if (mysqli_num_rows($mainsql) > 0) {  
			while ($maindata = mysqli_fetch_array($mainsql)) {  
				...
				if($curr_user_row['user_status'] == '2')
				{
					$order_form_link = $order_form_api_path.'dispatch_order.php?id='.$maindata['id'];
				}
				else
				{
					$order_form_link = '';
...
while ($row = mysqli_fetch_array($result)) {
	...
	if($curr_user_row['user_status'] == '2')
	{
		$order_form_link = $order_form_api_path.'pending_order.php?order_id='.$order_id;
	}
	else
	{
		$order_form_link = '';
...
if(isset($_POST['notification_list']) && isset($_POST['user_id'])){
	...
	if(mysqli_num_rows($query_cart)>0)
	{
		while ($data = mysqli_fetch_array($query_cart)) {
			...
			if($data['image'] !='')
			{
				$row['image'] = $file_path.'/notification/'.$data['image'];
			}
			else
			{
				$row[ 'image' ] = '';
...
if(isset($_POST['product_pdf'])){
    ...
    if (mysqli_num_rows($mainsql) > 0) {
        ...
        $set = [
            'status' => true,
            'pdf_file' => $file_path . 'PDF/' . $maindata['file'],
            'date' => $maindata['date'],
        ];
    } else {
